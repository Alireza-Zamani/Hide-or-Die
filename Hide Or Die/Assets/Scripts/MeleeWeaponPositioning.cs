...
public class MeleeWeaponPositioning : MonoBehaviourPunCallbacks
{
    ...
    private void Update()
    {
        ...
        {
            target = meleeWeaponTargeting.GetMeleeTarget();
            if (target)
            {
    ...
    {
        float newAngle;
        newAngle = this.transform.rotation.eulerAngles.z;
        newAngle -= 60; ;
    ...
    {
        Vector2 vectorRelativeToTarget = target.transform.position - this.transform.position;
        float angle = Mathf.Atan2(vectorRelativeToTarget.y, vectorRelativeToTarget.x) * Mathf.Rad2Deg;

        if (parent.localScale.x < 0 && this.transform.localScale.x > 0)
        {
        ...
        else if (parent.localScale.x > 0 && this.transform.localScale.x < 0)
        {
            var xScale = this.transform.localScale.x;
